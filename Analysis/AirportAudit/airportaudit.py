...
delayedflights=delays[delays['ArrDelay']>=15]

# print delayedflights
A=delayedflights.groupby('DestAirportID').count().sort('Month')['Month']
B=delays.groupby('DestAirportID').count().sort('Month')['Month']

C=pd.concat([A,B],axis=1)

C.to_csv('A.csv')
# B=delays.groupby('DestAirportID').count().sort('Month').tail(15)

# res = A.set_index('DestAirportID') / B.set_index('DestAirportID')
